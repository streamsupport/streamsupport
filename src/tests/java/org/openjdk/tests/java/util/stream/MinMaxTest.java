...
public class MinMaxTest extends OpTestCase {
    public void testMinMax() {
        assertTrue(!StreamSupport.stream(countTo(0)).min(java8.lang.Integers::compare).isPresent());
        assertTrue(!StreamSupport.stream(countTo(0)).max(java8.lang.Integers::compare).isPresent());
        assertEquals(1, (int) StreamSupport.stream(countTo(1000)).min(java8.lang.Integers::compare).get());
        assertEquals(1000, (int) StreamSupport.stream(countTo(1000)).max(java8.lang.Integers::compare).get());
    }

    @Test(dataProvider = "StreamTestData<Integer>", dataProviderClass = StreamTestDataProvider.class)
    public void testOps(String name, TestData.OfRef<Integer> data) {
        exerciseTerminalOps(data, s -> s.min(java8.lang.Integers::compare));
        exerciseTerminalOps(data, s -> s.max(java8.lang.Integers::compare));
    }
