...
// CVS rev. 1.119
    ...
    private int setDone() {
        int s;
        if (((s = ForkJoinPool.getAndBitwiseOr(this, STATUS, DONE)) & SIGNAL) != 0)
    ...
    private int abnormalCompletion(int completion) {
        for (int s, ns;;) {
            if ((s = status) < 0)
                return s;
            else if (U.compareAndSwapInt(this, STATUS, s, ns = s | completion)) {
                if ((s & SIGNAL) != 0)
                    synchronized (this) { notifyAll(); }
                return ns;
            }
    ...
    final void internalWait(long timeout) {
        if (ForkJoinPool.getAndBitwiseOr(this, STATUS, SIGNAL) >= 0) {
            synchronized (this) {
                if (status >= 0)
    ...
    private int externalAwaitDone() {
        ...
        if (s >= 0 && (s = ForkJoinPool.getAndBitwiseOr(this, STATUS, SIGNAL)) >= 0) {
            ...
            if (t instanceof ForkJoinWorkerThread) {
                ...
                while ((s = status) >= 0 &&
                    ...
                    if ((ms = TimeUnit.NANOSECONDS.toMillis(ns)) > 0L &&
                        (s = ForkJoinPool.getAndBitwiseOr(this, STATUS, SIGNAL)) >= 0) {
                        synchronized (this) {
                            if (status >= 0)
